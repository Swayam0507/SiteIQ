...
class SQLiteCursorWrapper:
    ...
    def execute(self, query: str, params=()):
        ...
        q = q.replace("TO_CHAR(login_date, 'YYYY-MM-DD')", "login_date")
        q = q.replace("TO_CHAR(login_time, 'HH24:MI:SS')", "login_time")
        q = q.replace("TO_CHAR(signup_date, 'YYYY-MM-DD')", "signup_date")
        q = q.replace("TO_CHAR(signup_time, 'HH24:MI:SS')", "signup_time")
        q = q.replace("TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS')", "created_at")
        q = q.replace("TO_CHAR(lh.login_date, 'YYYY-MM-DD')", "login_date")
        q = q.replace("TO_CHAR(lh.login_time, 'HH24:MI:SS')", "login_time")
        q = q.replace("TO_CHAR(u.created_at, 'YYYY-MM-DD HH24:MI:SS')", "created_at")
        q = q.replace("TO_CHAR(MAX(login_date), 'YYYY-MM-DD')", "MAX(login_date)")
        q = q.replace("RETURNING *", "")
        try:
            self._c.execute(q, params)
        except sqlite3.OperationalError as e:
            if "duplicate column name" in str(e) or "syntax error" in str(e):
                pass
            else:
                raise
...
class SQLiteConnWrapper:
    def __init__(self, db_path="data/siteiq.db"):
...
def _resolve_connection_string() -> str:
    """Try NeonDB. If unreachable, try local DB. If still unreachable, fall back to SQLite."""
    ...
    if _PRIMARY_DB:
        try:
            ...
            print(f"[DB] NeonDB unreachable ({type(e).__name__}) — trying local PostgreSQL.")
    try:
        conn = _safe_connect(_LOCAL_DB_URL, connect_timeout=2)
        conn.close()
        print("[DB] Active connection: Local PostgreSQL")
        return _LOCAL_DB_URL
    except Exception as e:
        print("[DB] PostgreSQL unavailable — falling back to local SQLite database (data/siteiq.db).")
        _USING_SQLITE = True
        return "sqlite://data/siteiq.db"
...
def create_user(name: str, email: str, password: str) -> dict:
    ...
    try:
        conn = get_db()
        c = conn.cursor(cursor_factory=psycopg2.extras.DictCursor) if not _USING_SQLITE else conn.cursor()
        c.execute(
        ...
        user = c.fetchone()
        if user is None and _USING_SQLITE:
        ...
        if conn:
            conn.rollback()
        if "UNIQUE constraint failed" in str(e) or "already registered" in str(e):
            raise HTTPException(status_code=400, detail="Email already registered")
...
def get_user_history(user_id: int, limit: int = 20) -> list:
    ...
    for r in rows:
        ...
        # PostgreSQL/SQLite datetime to ISO string
